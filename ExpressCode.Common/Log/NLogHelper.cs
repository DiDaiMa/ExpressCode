...
namespace ExpressCode.Common
{
    public class NLogHelper : INLogHelper
    {
        ...
        public static void AddAOPLog(IInvocation invocation, string result, bool way)
        {
            ...
            aopNlog.ReturnTime = DateTime.Now;
            ...
            if (way == true)
            {
                ...
                "返回结果:" + result;
                NLogHelper.Debug(message);
            }
            //数据库写入
            //else
            //{
            //    string sql = "insert into AopNlog values(@MethodName,@ReturnType,@ReturnUrl,@ReturnResult,@ReturnTime)";
            //    int i = _aopNlogRepository.Execute(sql, new { @MethodName = aopNlog.MethodName, @ReturnType = aopNlog.ReturnType, @ReturnUrl = aopNlog.ReturnUrl, @ReturnResult = aopNlog.ReturnResult, @ReturnTime = aopNlog.ReturnTime });
            //}
        }
